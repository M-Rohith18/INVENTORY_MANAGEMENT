...
from .serializer import AddItemSerializer, ForgotPasswordSerializer, RegisterSerializer,CategoryAddSerializer, CategoryListSerializer, ItemDetailSerializer, ItemEditSerializer, ItemListSerializer, ItemSerializer, ResetPasswordSerializer, StockTransactionListSerializer
import jwt
...
class ItemListViewAPIView(JWTAuthenticationMixin, ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = Item.objects.filter(user=user, status='active')

        item_name = self.request.GET.get('item_name', '').strip()
        ...
        if item_name:
            queryset = queryset.filter(
                ...
                Q(category__name__icontains=item_name)
            )
...
class ItemSoftDeleteAPIView(JWTAuthenticationMixin, APIView):
    def delete(self, request, pk):
        ...
        try:
            item = Item.objects.get(pk=pk, user=user)
        except Item.DoesNotExist:
            return Response({"detail": "Item not found."}, status=status.HTTP_404_NOT_FOUND)

        item.status = 'inactive'
        item.save()
        return Response({"detail": "Item marked as inactive."}, status=status.HTTP_200_OK)
    

class ViewItemAPIView(JWTAuthenticationMixin, APIView):
...
# Template view
def edit_item_template(request, pk):
    return render(request, 'edit.html', {'item_id': pk})
...
class EditItemAPIView(APIView):
    def get(self, request, pk):
        ...
        try:
            item = Item.objects.get(id=pk, user=user)
        except Item.DoesNotExist:
            return Response({'detail': 'Item not found'}, status=status.HTTP_404_NOT_FOUND)

        serializer = ItemEditSerializer(item)
        return Response(serializer.data)
    ...
    def put(self, request, pk):
        ...
        serializer = ItemEditSerializer(item, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def download_reports(request):
    ...
    # Header
    writer.writerow(['Item', 'Type', 'Quantity', 'Date'])
